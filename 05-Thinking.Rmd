...
:::source
This chapter is adapted nearly verbatim from Matt Crump's "Answering Questions with Data." https://www.crumplab.com/statistics/thinking-about-answering-questions-with-data.html 
:::
...
```{r 12effectdists, fig.cap="Each panel shows hypothetical distributions for two conditions. As the effect-size increases, the difference between the distributions become larger"}
...
```{r 12effectdistsB, fig.cap="Each panel shows hypothetical distributions for two conditions. As the effect-size increases, the difference between the distributions become larger"}
...
```{r 12powercurve, fig.cap="This figure shows power as a function of effect-size (Cohen's d) for a between-subjects independent samples t-test, with N=10, and alpha criterion 0.05."}
...
```{r 12powercurveN, fig.cap="This figure shows power as a function of N for a between-subjects independent samples t-test, with d=0.2, and alpha criterion 0.05."}
...
```{r, 12effectsizeType1, fig.cap="Effect size as a function of p-values for type 1 Errors under the null, for a paired samples t-test."}
...
```{r 12cohensD, fig.cap="Each panel shows a histogram of a different sampling statistic", eval=FALSE}
all_df<-data.frame()
for(i in 1:10000){
