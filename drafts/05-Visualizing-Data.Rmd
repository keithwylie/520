# Visualizing Data {#Data-Viz}

:::quote
Mankind invented a system to cope with the fact that we are so intrinsically lousy at manipulating numbers. It's called the graph.
`r tufte::quote_footer('--- Charlie Munger')`
:::

:::source
Part of this chapter is adapted from Matthew Crump's **Descriptive Statistics** chapter in "Answering Questions with Data." https://www.crumplab.com/statistics/
:::

:::fact
Testing the fun fact box. Find some fun facts above descriptives from UNBOUND Gravel.
:::
...
This chapter is about **descriptive statistics**. These are tools for describing data. Some things to keep in mind as we go along are:
  ...
  1. Describing data is necessary. Usually, we have way more information than we can process without summarizing.
...
UNBOUND Gravel is an annual gravel bike race through the Flint Hills of Kansas. Each year, thousands of cyclists visit Emporia, Kansas. Business owners and community leaders might want to know more about who is visiting their town, so that they can advertise accordingly. Knowing the ages of the people traveling to Emporia for the UNBOUND Gravel might help the city plan events that are appropriate and exciting for that age group. So, let's say we know the ages of all the people registered to participate in the UNBOUND Gravel next year. The data might look something like this:
...
The dots have disappeared, and now we some bars. Each bar is a summary of the dots, representing the number of dots (frequency count) inside a particular range of happiness, also called **bins**. For example, how many people between ages 40 and 50? The sixrh bar, the one between 40 and 50 on the x-axis, tells you how many. Look how tall that bar is. How tall is it? The height is shown on the y-axis, which provides a **frequency** count (the number of dots or data points). It looks like around 70 or so cyclists are in their 40s.
...
g1 <- qplot(x=gravel, color="white", bins=5)+
  theme_classic()+
  theme(legend.position="none")

g2 <- qplot(x=gravel, color="white", bins=10)+
  theme_classic()+
  theme(legend.position="none")

g3 <- qplot(x=gravel, color="white", bins=20)+
  theme_classic()+
  theme(legend.position="none")

g4 <- qplot(x=gravel, color="white", bins=50)+
  theme_classic()+
  theme(legend.position="none")
